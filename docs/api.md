...
| `set_theme` | `theme`: `manuscript` \| `blueprint` \| `sketchbook` | Stamp `data-cfy-theme="<theme>"` on the artifact's root element, re-skinning it to the app's live theme setting (epic conceptify-89k; artifact-spec §1.5). The serve-time bridge tag already carries the theme current at load (applied pre-first-paint, overriding the authored stamp); this message covers live `settings-changed` while an artifact is open, and the shell re-sends it on every `ready` because immutable version responses can be cached with a stale serve-time stamp. Unknown ids are ignored (closed-set validation in the bridge). |
